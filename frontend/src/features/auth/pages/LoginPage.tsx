...
export default function LoginPage() {
  const navigate = useNavigate() // Para las rutas de navegación
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    // Ejemplo para probar los login con correos
    if (email === 'admin@uach.cl' && 'grandelau') {
      navigate('/admin')
    } else if (email === 'director@uach.cl' && 'grandelau') {
      navigate('/director')
    } else if (email === 'decano@uach.cl' && 'grandelau') {
      navigate('/decano') 
    } else if (email === 'autoridad@uach.cl' && 'grandelau') {
      navigate('/autoridad')
    } else {
      ...
      <section className="flex min-h-screen items-center justify-center px-5 py-10 sm:px-8 lg:px-12">
        <div className="w-full max-w-[460px]">
          ...
          <div className="rounded-[20px] border border-slate-200 bg-white p-6 shadow-xl shadow-slate-200/50 sm:p-9">
            ...
            {mostrarError && (
              <div role="alert" className="mb-5 flex gap-3 rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-700">
                <span className="mt-0.5 size-2 shrink-0 rounded-full bg-red-500" aria-hidden="true" />
            ...
            <form className="space-y-5" onSubmit={handleSubmit}>
              ...
              <button 
                type="submit" 
                className="group flex h-12 w-full items-center justify-center gap-2 rounded-lg bg-[#FFB800] text-sm font-bold text-slate-900 shadow-md shadow-[#FFB800]/20 transition hover:bg-amber-500 focus:outline-none focus:ring-2 focus:ring-[#FFB800]/40 focus:ring-offset-2"
              >
                Iniciar Sesión
